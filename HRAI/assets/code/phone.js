"use strict";
/* ============================================================
   HRAI · phone.js — 手机端逻辑 / 触控（恐怖逃生精简版）
   （模式检测、摇杆、蹲/跳按钮、拖拽视角）
   收集 / 合成 / 开门：手机端靠近目标自动进行，无需按钮。
   ============================================================ */
...
/* ---------- 2b. 「使用」按钮（切换式：点一下开始收集/合成，再点停止） ---------- */
let touchUse=false;
function setUseUI(){
  const b=el('btnUse'); if(!b) return;
  b.textContent = touchUse ? '收集中' : '使用';
  if(touchUse) b.classList.add('on'); else b.classList.remove('on');
}
function toggleUse(){ touchUse=!touchUse; setUseUI(); }
(function(){
  const b=el('btnUse'); if(!b) return;
  b.addEventListener('click', e=>{ e.preventDefault(); e.stopPropagation(); if(!running||layoutEditing) return; toggleUse(); });
})();
setUseUI();

/* ---------- 3. 手机视角（#look 层拖拽；摇杆与按钮在其上层，互不干扰） ---------- */
