...
# 全局错误捕获：任何 JS 错误都显示到页面上
error_handler = r"""
window.onerror = function(msg, src, line, col, err) {
  var app = document.getElementById('app');
  if (app) {
    app.innerHTML = '<div class="err-page"><h1>️ JS 运行错误</h1><h2>' + escHtml(String(msg)) + '</h2><p>文件：' + escHtml(String(src)) + '<br>行号：' + (line||'') + '<br>列号：' + (col||'') + '<br><br><b>错误详情：</b><br><code style="display:block;white-space:pre-wrap;background:#1a1f2e;color:#ff6666;padding:16px;border-radius:8px;">' + escHtml(String(err&&err.stack||msg)) + '</code></p><a href="/wz.html" class="btn btn-primary">返回文章列表</a></div>';
  }
  return true;
};
"""
