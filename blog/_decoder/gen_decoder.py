...
# 读取模板并生成干净的 index.html
# 关键：JS 变量名只用 trimmed（7个字母），不用 trimmed（8个字母）
...
function renderInline(text) {
  text = text.replace(/`[^`]+`/g, function(m) { return '<code>' + m.substring(1, m.length - 1) + '</code>'; });
  text = text.replace(/\\*\\*[^*]+\\*\\*/g, function(m) { return '<strong>' + m.substring(2, m.length - 2) + '</strong>'; });
  text = text.replace(/\\*[^*]+\\*/g, function(m) { return '<em>' + m.substring(1, m.length - 1) + '</em>'; });
  return text;
}
...
with open('gen_decoder.py', 'w', encoding='utf-8') as f:
    f.write(py_script)

print('gen_decoder.py 已写入，现在运行它...')
