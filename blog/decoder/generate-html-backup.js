/**
 * generate-html-backup.js
 * 为每篇文章生成 html.html 备份文件（独立完整页面）
 * 运行：node generate-html-backup.js
 */
...
const ARTICLES_DATA_PATH = path.resolve(BLOG_ROOT, '..', 'articles-data.js');
...
// ========== 从 articles-data.js 读取 _pathMap ==========
function extractPathMap(filePath) {
  const content = fs.readFileSync(filePath, 'utf-8');
  const match = content.match(/_pathMap:\s*\{([\s\S]*?)\},/);
  if (!match) {
    console.error('❌ 无法从 articles-data.js 中提取 _pathMap');
    process.exit(1);
  }
  ...
  const re = /\s*"(\d+)"\s*:\s*"([^"]+)"/g;
  let m;
  while ((m = re.exec(mapStr)) !== null) {
    pathMap[m[1]] = m[2];
  }
  console.log('📋 提取到 ' + Object.keys(pathMap).length + ' 篇文章的路径映射');
  return pathMap;
}
...
function parseBlogFile(filePath) {
  ...
  while (i < lines.length) {
    ...
    if (line.charAt(0) === '[') {
      ...
      if (bracketEnd !== -1) {
        const key = line.substring(1, bracketEnd).toLowerCase();
        const val = line.substring(bracketEnd + 1).trim();
        meta[key] = val;
      }
    }
    i++;
  ...
  while (i < lines.length) {
    ...
    if (trimmed.toLowerCase() === '[/html]') {
      if (inHtml) {
        htmlBlocks.push(currentHtmlContent);
        currentHtmlContent = '';
        inHtml = false;
      }
      i++;
    ...
    if (trimmed.toLowerCase() === '[html]' || trimmed === '[HTML]') {
      if (textLines.length) {
        htmlBlocks.push('<p>' + escHtml(textLines.join('\n')) + '</p>');
        textLines = [];
      }
      inHtml = true;
      i++;
  ...
  return { meta, htmlContent: htmlBlocks.join('\n') };
}

// ========== 生成完整 HTML 页面 ==========
function generateHtmlPage(articleId, meta, htmlContent) {
  const title = meta.title || '无标题';
  const date = meta.date || '';
  const author = meta.author || 'ciallo0721-cmd';
  const tags = [];
  if (meta.tag) {
    tags.push(...meta.tag.split(/[,\s]+/).filter(t => t.length > 0));
  }

  const tagHtml = tags.length
    ? '<div class="article-tags">' + tags.map(t =>
        '<span class="article-tag">' + escHtml(t) + '</span>'
      ).join('') + '</div>'
    : '';

  const dateHtml = date
    ? '<div class="article-date">📅 ' + escHtml(date) + '</div>'
    : '';
...
<title>${escHtml(title)} - ciallo0721-cmd</title>
...
.article-body a:hover { color:var(--bili-pink); border-bottom-color:var(--bili-pink); }
.article-body ul, .article-body ol { margin:0 0 18px 30px; }
.article-body li { margin-bottom:8px; }
.article-body blockquote {
    border-left:4px solid var(--bili-blue); background:rgba(0,161,214,0.05);
    margin:20px 0; padding:16px 20px; border-radius:0 12px 12px 0;
    color:var(--text-secondary); font-style:normal;
}
.article-body blockquote p { margin-bottom:8px; }
.article-body blockquote p:last-child { margin-bottom:0; }
.article-body pre {
    background:#1a1f2e; color:#e4e4e4; padding:16px; border-radius:8px;
    overflow:auto; margin:20px 0; font-size:14px; line-height:1.5;
}
.article-body code {
    background:rgba(251,114,153,0.1); color:var(--bili-pink); padding:2px 6px;
    border-radius:4px; font-size:0.9em;
}
.article-body pre code { background:none; color:#e4e4e4; padding:0; }
.article-body img { max-width:100%; height:auto; border-radius:8px; margin:20px 0; box-shadow:0 2px 12px rgba(0,0,0,0.1); }
.article-body table { width:100%; border-collapse:collapse; margin:20px 0; }
.article-body th, .article-body td { border:1px solid #e0e0e0; padding:12px; text-align:left; }
.article-body th { background:linear-gradient(135deg, rgba(251,114,153,0.1), rgba(0,161,214,0.1)); font-weight:600; }
.article-body hr { border:none; height:1px; background:linear-gradient(135deg, var(--bili-pink), var(--bili-blue)); margin:30px 0; }
</style>
</head>
<body>
<div class="nav">
...
<div class="nav-links">
<a href="/blog/" class="back-btn"><i class="fas fa-arrow-left"></i> 文章列表</a>
</div>
</div>
</div>
...
<div class="article-num">文章 #${escHtml(String(articleId))}</div>
<h1 class="article-title">${escHtml(title)}</h1>
<div class="article-meta">
${dateHtml}
<div class="article-date">✍ ${escHtml(author)}</div>
</div>
${tagHtml}
</div>
...
// ========== 主流程 ==========
function main() {
  const pathMap = extractPathMap(ARTICLES_DATA_PATH);
  let successCount = 0;
  let failCount = 0;

  for (const [id, folderPath] of Object.entries(pathMap)) {
    const articleDir = path.join(BLOG_ROOT, folderPath);
    const blogFile = path.join(articleDir, id + '.blog');
    const outputFile = path.join(articleDir, 'html.html');

    if (!fs.existsSync(blogFile)) {
      console.warn(`⚠️  文章 ${id}：${blogFile} 不存在，跳过`);
      failCount++;
      continue;
    }

    try {
      const { meta, htmlContent } = parseBlogFile(blogFile);
      const pageHtml = generateHtmlPage(id, meta, htmlContent);
      fs.writeFileSync(outputFile, pageHtml, 'utf-8');
      console.log(`✅ 文章 ${id}（${meta.title || '无标题'}）→ html.html`);
      successCount++;
    } catch (err) {
      console.error(`❌ 文章 ${id} 生成失败：${err.message}`);
      failCount++;
    }
  }
